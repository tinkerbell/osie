...
discover_address=$(sed -nr 's|.*\bdiscover_address=(\S+).*|\1|p' /proc/cmdline)
if [ -z "$discover_address" ]; then
	discover_address="http://tinkerbell.$facility.packet.net"
fi
...
docker run --privileged -ti \
	-v /dev:/dev \
	--net host \
	quay.io/packet/discover-metal --send "$discover_address"
